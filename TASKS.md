...
### D-03 ✅ 已完成并归档（2026-09-17，commit `636e4df1`）→ 详见历史归档「D-03 项目总览文档」
...
### D-03 ✅ 项目总览文档：全项目唯一入口（2026-09-17 完成，commit `636e4df1`）

* **目标**：把"这份项目是啥"收敛到一个答案——此前 `CLAUDE.md`（工程视角）/ `README.md`（对外视角）/ `docs/ChatGPT规划输入-EIA项目介绍.md`（产品视角）三份重叠，且只有工程那份最新（09-14），产品那份停在 09-02，缺 T-12/T-16 之后的变动；新增 `docs/EIA项目总览.md` 作为唯一入口
* **状态**：✅ 已完成并归档
* **修改范围**：新建 `docs/EIA项目总览.md`（14 节）；改 `README.md`（索引 +1 行 + 表清单）、`docs/产品需求文档-PRD.md`（附录索引 + 页脚 + 计数）、`CLAUDE.md`（入口指针 + 数字校正）、`AGENTS.md`（入口指针 + 数字校正）、`docs/ChatGPT规划输入-EIA项目介绍.md`（标记"已被取代"）、`TASKS.md`。**禁止动**（已遵守）：一切代码——`app/` `tests/` `prompts/` `alembic/` `miniprogram/` `web/`，以及 docs/ 下其他文档正文
* **停止条件**：与源文档事实冲突超 3 处即停止汇报（防无限返工）——实际发现 5 处，均为源文档过期而非冲突，一并修正
* **验证数据**（全部当日实测，非估算）：
  * `information_schema.tables` @ eia-postgres = 27 张（含 `alembic_version`）→ **26 张业务表**（原口径 25，且清单漏 `eval_runs` / `user_wechat_bindings`）
  * `ls alembic/versions/*.py` = **16 个迁移**（至 016；原口径 13 版）
  * `grep -c "@router\."` = **49 个端点 / 11 个路由模块**（原口径 10 组 / 50 端点）
  * `grep -c '__tablename__' app/database/models.py` = 18 ORM 模型（+8 张迁移建表 = 26）
  * `app/config.py` 确认 canary `13:05`、告警兜底 `12:45`
  * `AGENTS.md` "10 个 Agent 协作" → 11；PRD 附录计数：决策记录 10→18 条、Prompt 迭代 5→11 次、BadCase 3→5 个
  * 总览内 41 个相对链接全部可解析（脚本逐条 `[ -e ]` 校验，0 MISS）
* **一条被证伪的线索（勿再当漂移改）**：`CLAUDE.md` 决策速查表"6 之后直接跳到 14"初判为编号漂移，核查后确认**是对的**——它取的是 `AI产品经理面试作战包.md` 的 16 条编号（决策 14 = n8n vs cron），与 `关键产品决策记录.md` 的 18 条是**两套独立编号**。处置：不改编号，改为在总览 §7 标注"按标题检索，不要按编号对号入座"
* **防漂移产物**：总览 §0 权威源对照表（每类事实指向唯一权威源，避免总览变成第 4 个事实源）+ §14.2 同步清单（6 类易漂数字 × 各自出现位置）
