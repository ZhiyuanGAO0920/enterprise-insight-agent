...
### D-03 ✅ 已完成（2026-09-17）项目总览文档：全项目唯一入口

* **目标**：合并三份重叠文档——`CLAUDE.md`（工程视角）+ `README.md`（对外视角）+ `docs/ChatGPT规划输入-EIA项目介绍.md`（产品视角）——为一份 `docs/EIA项目总览.md`，作为全项目唯一入口；并在 README / PRD 附录 / CLAUDE.md 建立入口指针，避免多入口各说各话

* **状态**：✅ 已完成（待提交后移入历史归档）

* **修改范围**：新建 `docs/EIA项目总览.md`；改 `README.md`（文档索引 +1 行）、`docs/产品需求文档-PRD.md`（附录索引 +1 行、页脚日期）、`CLAUDE.md`（入口指针 + 实测数字校正）、`docs/ChatGPT规划输入-EIA项目介绍.md`（顶部标记"已被取代"）、`TASKS.md`（本登记）。**禁止改** 一切代码（`app/` `tests/` `prompts/` `alembic/` `miniprogram/` `web/`）与 docs/ 下其他文档正文

* **停止条件**：总览一次成型；与源文档事实冲突超 3 处即停止汇报（防无限返工）

* **验证数据**：PG 实测 `information_schema.tables` = 27 张（含 `alembic_version`）→ **26 张业务表**；`ls alembic/versions/*.py` = **16 个迁移**（至 016）；`grep -c "@router\."` = **49 个端点 / 11 个路由模块**；`app/config.py` 确认 canary 13:05、alert 兜底 12:45。据此校正 4 处过期口径（原"25 张表""13 版迁移""10 路由组/50 端点"）。防漂移同步清单见总览 §14.2
