...
8. [质量监控（AI 质量监控面板）](#8-质量监控ai-质量监控面板)
9. [异常与降级处理](#9-异常与降级处理)
10. [边界情况汇总](#10-边界情况汇总)
...
用户点击「📄 PDF」
  ...
  │
  ├─ [200] → 浏览器下载 .pdf 文件
  └─ [非 200] → 前端轻提示"PDF 服务不可用，已降级为 Markdown 下载"
...
查看反馈历史：GET /api/v1/feedback/history（V4.5 新增）
  └─ 弹窗展示过往反馈记录列表（含质检徽标 V4.6.4）

意见反馈（不关联分析记录）：POST /api/v1/feedback/contact（移动端入口）
  └─ 写入 user_feedback，rating='contact' 区分来源（V4.6.7 起管理端可见）

管理端反馈明细（V4.6.7 新增）：
GET /api/v1/feedback/admin-list（admin/区域经理权限）
  └─ Admin「反馈统计」Tab 明细表格：时间 / 用户 / 类型（彩色标签）/ 内容 / 关联问题
  └─ /stats 只给聚合数字，反馈原文明细由此接口下钻
```
...
> 监控页设计原则见 [监控页指标设计原则.md](监控页指标设计原则.md)（决策台而非数据仓库：13 项候选指标裁决）。V4.6.3 起「三态口径」、V4.6.4 起「四维分布 / 兜底下钻 / 反馈徽标」、V4.6.5 起「最近错误降噪」、V4.6.6 起「Hero 四象限」。

### 正常流程

```
侧边栏点击「监控」→ 进入 AI 质量监控页
  │
  ├─ 时间范围 pills：7 天 / 30 天 / 本月 / 上月 / 自定义日期（V4.6.6 自定义弹窗）
  │   └─ GET /api/v1/monitor/overview?days=N（后端 60s Redis 缓存 + 前端 30s 缓存）
  │
  ├─ Hero 四卡（量·质·速·稳，V4.6.6）：
  │   ├─ 📊 日均分析量（状态：≥90 优秀 / ≥75 良好 / 需关注）
  │   ├─ ✅ Reflection 通过率（三态口径，V4.6.3）
  │   │   └─ 副行「好评率 X% · 未过 N 条 · 解析兜底 N 条」
  │   │       └─ 点击「解析兜底 N 条」→ GET /monitor/reflection-fallback → 弹窗明细列表
  │   │           └─ ⚠️ 警示「兜底 ≠ 质检通过：这些报告质量未经验证，建议抽查复读」
  │   ├─ ⚡ P50 响应延迟（副行：P95 · 完整分析耗时）
  │   └─ ⚠️ 异常会话（副行：事件 N 条 · 占会话 X%）
  │       └─ 整卡可点击 → 平滑滚动定位到「最近错误」板块
  │
  ├─ 质量指标组：重试率（+修复率）/ 修复率 / 用户好评率 / 完整分析 P90
  ├─ 成本指标组：单任务成本（¥0.02x/次，V4.6.6）/ 日均成本 / 月均成本
  ├─ 🤖 Agent 健康度：错误率排行表（运行 / 错误 / 错误率 / 平均 / 最大）
  ├─ ❌ 最近错误（V4.6.5 降噪）
  │   ├─ 摘要头「共 N 条 · 超时 X · SQL Y · 其他 Z」（三类与技术异常图标同源）
  │   ├─ 默认只渲染前 10 行 → 「展开全部 N 条」原位展开 / 再收起
  │   └─ 两类错误合并：技术异常（agent_trace_events.error 非空）+ 质检未过（reflection_passed=false，还原为「质检未通过[维度×N]」摘要）
  ├─ 🔬 质检未过原因分布（V4.6.4 独立板块）
  │   ├─ ECharts 横向条形图：一致性 / 逻辑 / 可操作性 / 完整性
  │   └─ 全过时显示「✅ 全部通过质检」（不渲染图表）
  └─ 📊 Token 消耗趋势：折线图 + 累计 Input / Output / 总成本
```
...
| 场景 | 触发条件 | 系统行为 |
|------|---------|---------|
| overview 后端异常 | SQL 查询失败 | `logger.exception` 记录；前端显示「❌ 加载失败」 |
| Redis 不可用 | 缓存读写异常 | 跳过缓存直接查库（overview 60s 缓存降级） |
| 日期参数非法 | start_date 晚于 end_date / 格式错误 | 返回 400 + 明确错误信息 |
| 兜底明细加载失败 | `/monitor/reflection-fallback` 异常 | 弹窗提示失败，页面其他板块不受影响 |
| 无技术异常记录 | 近 30 天无 error 事件 | 「✅ 无错误记录」 |
| 质检全部通过 | 无 `reflection_passed=false` 记录 | 原因分布板块显示「✅ 全部通过质检」 |
| 无数据权限 | store_ids = [] | 相关统计返回空 |

### 边界情况

| 情况 | 行为 |
|------|------|
| 三态口径边界（V4.6.3） | 通过率 = passed/total，但 DB 无 `reflection_feedback` 列，「简单查询跳过质检」与「真过」不可区分——如实标注；未过与解析兜底可区分（兜底标记 `LIKE '%Reflection did not return structured result%'`） |
| 重试率口径（V4.6.2 修复） | 只统计 report_agent 事件（report_runs ≥ 2 判重试），reflection 事件不重复计数——否则正常会话恒被判重试 |
| 异常会话口径（V4.6.6） | 异常会话 = 至少一次 error 事件的会话；技术异常实测全为超时，多数经重试自愈仍产出报告——不夸大 |
| 反馈历史徽标（V4.6.4） | 反馈弹窗历史中 `reflection_passed=false` 显示「⚠️ 质检未过」徽标，hover 注明「V4.6.2 起简单查询跳过质检，DB 无法区分跳过与未过」 |
| 自定义日期范围 | 按实际天数统计（period_days 动态变化），前端 30s 缓存键包含日期参数 |
| 展开/收起最近错误 | 30s 缓存未过期时直接重渲染；过期则重新请求、加载完成后按新开关状态渲染 |
...
| `/api/v1/feedback/history` | POST | Bearer | 反馈历史（含质检徽标字段） |
| `/api/v1/monitor/overview` | GET | Bearer | 质量总览（Hero 四象限 / 三态 / 四维分布 / 成本 / 重试 / 延迟） |
| `/api/v1/monitor/errors` | GET | Bearer | Agent 错误日志（技术异常 + 质检未过合并） |
| `/api/v1/monitor/reflection-fallback` | GET | Bearer | 解析兜底报告明细（V4.6.4 下钻） |
| `/api/v1/monitor/feedback` | GET | Bearer | 反馈明细（含 reflection_passed） |
