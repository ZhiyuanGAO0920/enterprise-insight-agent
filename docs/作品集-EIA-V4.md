...
## 3. Demo 故事线——从问题到答案的 60 秒

### 场景：连锁零售老板的周一早晨

> "上周华东区销售为什么下降了？哪些门店需要重点关注？"

#### 第 1 步：全局看板（30 秒掌握全局）
登录后进入经营看板——6 个 KPI 卡片带环比箭头、30 天趋势折线图、区域占比环形图、
门店销售额 Top 10 + 退款率 Top 10。一眼看清"哪里好、哪里不好"。

#### 第 2 步：提问分析（60 秒获得报告）
输入"各区域经营对比" → Supervisor Agent 自动规划路由 → 
5 个领域 Agent 并行分析（销售/CRM/财务/库存/供应链）→
Chart Advisor 推荐图表 → Report Agent 生成结构化报告 →
Reflection Agent 4 维质检 → 输出最终报告。

#### 第 3 步：追问与反馈
报告底部自动生成追问建议，点一下继续深入分析。
提交反馈后可见平台好评率，形成质量闭环。

#### 第 4 步：切换角色验证 RLS
切换到 lisi（店长）→ 系统自动过滤数据范围，仅显示该店长权限内的门店。
同一系统，不同角色看到不同的数据——行级安全（RLS）自动生效。

#### 第 5 步：质量监控（AI 运维）
管理员查看 AI 质量监控面板：Reflection 通过率、Token 成本、Agent 错误率、延迟 P50/P95。
系统不仅能用，还能自我监控和持续优化。
...
| 层 | 方式 | 规模 |
|----|------|------|
| 离线评估集 | 20 条标准问题，覆盖查询型/分析型 | 每次 Prompt 修改后跑 |
