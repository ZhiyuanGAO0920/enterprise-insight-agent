...
**金丝雀闭环（V4.7）**：评估体系之上再加一道"元监控"——外部 LLM 模型漂移是"无通知、渐进式"的，供应商推新版本后 Prompt 输出可能悄悄变差。每次评估结果落库 `eval_runs`（带模型版本），16 条固定子集每日 09:30 自动跑（n8n 定时触发），与上一次**同模型**基线对比：通过率下降 > 5% / 维度覆盖率下降 > 10% / 延迟上升 > 5s / Reflection 严格通过率下降 > 8% 即触发漂移告警。
