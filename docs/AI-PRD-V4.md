...
| **模型版本锁定 + 金丝雀** | eval 落库带 `model_version` 维度；每周定时跑固定子集对比基线（每日 13:05 兜底检查，7 天幂等窗口），超阈值告警——已落地 | ✅ V4.7 已实施（`eval_runs` 表 + `/api/v1/eval/canary` + 应用内定时 + n8n 双保险） |
...
| 模型漂移（金丝雀） | 每周 eval 跑分，通过率下降 > 5% / Reflection 严格通过率下降 > 8% 告警 | ✅ V4.7 已实施（eval_runs 落库 + 应用内定时每日 13:05 兜底检查、7 天幂等窗口 + n8n 双保险 + drift 信号推送） |
...
| 效果随时间衰减（Prompt/模型漂移） | 中 | 中 | ✅ 金丝雀闭环（V4.7：eval 落库带 model_version + 每周自动跑分对比基线，超阈值告警；每日 13:05 兜底检查） |
