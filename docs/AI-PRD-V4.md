...
| **模型版本锁定 + 金丝雀** | eval 落库带 `model_version` 维度；每日定时跑固定子集对比基线，超阈值告警——已落地 | ✅ V4.7 已实施（`eval_runs` 表 + `/api/v1/eval/canary` + n8n 模板） |
...
| 模型漂移（金丝雀） | 每日 eval 跑分，通过率下降 > 5% / Reflection 严格通过率下降 > 8% 告警 | ✅ V4.7 已实施（eval_runs 落库 + n8n 每日触发 + drift 信号推送） |
...
| 效果随时间衰减（Prompt/模型漂移） | 中 | 中 | ✅ 金丝雀闭环（V4.7：eval 落库带 model_version + n8n 每日跑分对比基线，超阈值告警） |
