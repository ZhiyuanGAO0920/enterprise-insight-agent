...
| v1.3 | 2026-08-03 | 高志远 | 复核同步：11 节点 / 49 端点 / 192 测试 / 13 迁移（V4.6 已入库） |
...
- **输入**：用户问题 + 客户数据库表结构（自动发现 + YAML 映射） + RAG 检索的历史 SQL
...
- **检索**：新问题先查历史相似分析，结果注入 Agent Prompt 作为少样本示例
...
| v4.5 | V4.5 | 前端重构 + 性能优化 + 反馈闭环 | + 热重载 + 排名关键词注入 + 客户数据库结构动态适配 | SQL 截断问题修复，适配效率提升 |
...
| FR-014 | 客户数据库结构适配 | P0 | 3 层适配（自动发现 → YAML 映射 → Prompt 动态生成） |
...
- [ ] 数据存储加密（AES-256）— 待实施
- [ ] PII 脱敏（手机号、身份证号）— 待实施（⭐ 接大客户前必修，见下方"必修项"）
...
**⭐ 接大客户前必修项**（2026-08-10 第三方 AI 对照《AI产品经理核心知识手册》审查结论，已逐条核实代码与评测数据属实）：

| 必修项 | 具体方案 | 状态 |
|-------|---------|------|
| **PII 脱敏** | SQL 结果回传 LLM 前按 `customer_schema.yaml` 标记的 PII 列（姓名/手机号/身份证/邮箱）做掩码；Report Prompt 增加"不得还原脱敏字段"约束。个保法"最小必要"原则的合规硬伤，接金融/医疗/政府客户一票否决项 | ❌ 待实施（改 `sql_runner` + Prompt，约 1-2 天） |
| **成本保护硬限额** | 单次 Token 上限（超限截断 context 或拒答）+ 租户月限额（复用多租户骨架）+ 成本预警 + 达限额降级小模型——当前只有速率限制与成本监控（"看得到"），没有拦截（"拦得住"） | ❌ 待实施 |
| **模型版本锁定 + 金丝雀** | eval 落库带 `model_version` 维度；每周定时跑固定子集对比基线（每日 09:30 兜底检查，7 天幂等窗口），超阈值告警——已落地 | ✅ V4.7 已实施（`eval_runs` 表 + `/api/v1/eval/canary` + 应用内定时 + n8n 双保险） |
| **Reflection 维度重构** | 质检与用户满意度零相关（V4.6.3 实测 28% vs 25%），四维偏文字层（可操作性仅 4%）——维度向"可操作性/业务价值"对齐后重测相关性 | 🚧 小步验证中 |
...
| 单次 Token 上限 | > 60K 超限截断 context 或拒答 | ❌ 待实施（⭐ 接大客户前必修：成本保护"拦得住"） |
| 租户月成本限额 | 超限告警 + 达限额降级小模型 | ❌ 待实施（⭐ 接大客户前必修，复用多租户骨架） |
| 模型漂移（金丝雀） | 每周 eval 跑分，通过率下降 > 5% / Reflection 严格通过率下降 > 8% 告警 | ✅ V4.7 已实施（eval_runs 落库 + 应用内定时每日 09:30 兜底检查、7 天幂等窗口 + n8n 双保险 + drift 信号推送） |
...
| db_schema | String(50) | 每租户独立数据库结构模式 |
...
| **V3.1** | 领域扩展 | 库存/供应链 Agent + 数据库结构适配层 + RAG + 137 测试 |
...
| 成本超预算（高频使用） | 中：影响 ROI | 低 | ✅ Redis 缓存 + V4 单次图执行 + 排名注入减少 SQL 行数；🚧 成本硬限额（单次 Token 上限 + 租户月额，接大客户前必修） |
| 客户数据库表结构差异大 | 高：无法开箱即用 | 高 | ✅ 3 层适配（自动发现 → YAML 映射 → Prompt 动态生成） |
| 数据泄露/安全事故 | 高：法律风险 | 低 | ✅ 私有化部署 + SQL 注入防护 + XSS 防御 + 审计日志；🚧 PII 脱敏（SQL 结果回传 LLM 前掩码，接大客户前必修） |
| 效果随时间衰减（Prompt/模型漂移） | 中 | 中 | ✅ 金丝雀闭环（V4.7：eval 落库带 model_version + 每周自动跑分对比基线，超阈值告警；每日 09:30 兜底检查） |
...
| v4.5 | V4.5 | 前端重构 + 性能优化 + 反馈闭环 | + 热重载 + 排名关键词注入 + 客户数据库结构动态适配 | SQL 截断修复，适配效率提升 |
