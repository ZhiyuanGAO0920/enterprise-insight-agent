...
V4.6.3 三态 hero 卡片之后，监控页仍有三块质检信号缺失：未过原因的维度分布（接口已有数据但未渲染）、兜底报告只有数字无法下钻、反馈历史看不到质检状态。本轮全部补全。
...
**实测验证**（真实服务 localhost:8002 + 真实 DB）：
- 近 30 天四维分布：**完整性 40（50%）> 一致性 28（35%）> 逻辑 9（11%）> 可操作性 3（4%）**——与 V4.6.3「质检零相关」发现互证：质检抓的大头是文字层面的完整性/一致性，用户最在意的可操作性维度几乎无把关（4%），是质检重构的直观依据
- 兜底明细端点实测：共 32 条全部可查（问题 + 时间），点击 hero 数字弹窗下钻 ✓
- 反馈徽标 mock 验证：`reflection_passed=false` 显示徽标、`true` 不显示，条件渲染正确 ✓
- 浏览器实测监控页：四维卡、可点击兜底数、弹窗列表均正常渲染（console 无新增报错）
...
### ✨ 优化：交叉校验升级为「算术审计」+ 反馈闭环 + 检索评估 + Reflection 对照实验

| # | 文件 | 优化 |
|---|------|------|
| 1 | `tests/run_eval.py` | **派生值重算校验**：报告数字两级校验——直接匹配（源数据/问题/合计/均值，0.5% 容差）+ **派生匹配**（用源数据重算差/比率/百分率/增长百分率/乘积/均值）。LLM 合法算出的客单价、占比、增长率、合计都能通过算术审计；编造数字几乎不可能与任意真值的差/比/积在 0.5% 内重合。实测 Q52 报告（全部派生数字）0.29 → 0.87，补全数据后 1.0，虚构「2.6 亿」仍被抓 |
| 2 | `scripts/analyze_feedback.py` | **反馈闭环脚本**：评分分布 + 按 Agent 不准确率 + **三角验证**（评估集问题被线上标记不准确的分布，BadCase 直指评估用例）+ **关联分析**（不准确率 × 质检状态 / × 是否评估集问题）+ 坏例 → 评估集候选（--apply 入库，与现有 102 条自动去重） |
| 3 | `scripts/eval_retrieval.py` | **检索质量评估**（零 LLM 成本）：12 组改写句对测语义召回 recall@5 + RAG SQL 命中率，把「BGE-M3 召回率 ~79%」从估算变实测 |
| 4 | `app/tools/memory.py` | **🐛 修复嵌入语义缺陷**（检索评估发现）：`save_analysis_history` 此前把**报告全文**做嵌入存库，而检索侧用**问题**做向量——短查询对长文做余弦检索，改写句召回率实测仅 25%。改为嵌入问题文本 |
| 5 | `scripts/backfill_embeddings.py` | 存量向量回填：全部 778 条按问题文本重嵌（含此前 embedding 为 NULL 的记录——保存时 Ollama 不可用导致嵌入失败，评估集问题大量缺向量，是召回虚低的另一主因） |
| 6 | `app/workflow/state.py` / `app/workflow/graph.py` / `app/api/routes/analysis.py` / `tests/run_eval.py` | **Reflection 对照实验**：`/analyze` 与 `/analyze-stream` 新增 `skip_reflection` 实验参数（绕过缓存保证真实执行、不污染生产缓存），graph `after_report` 支持跳过质检与重试；`run_eval.py --skip-reflection` 一键跑对照，结果标记 `ablation` 状态 |
| 7 | `scripts/eval_subset.py` | 支持 `--skip-reflection` 传参 |

**实测结果与发现**（真实数据）：
- **检索召回率 25% → 75%**（top-1 平均相似度 0.669 → 0.847）——嵌入语义修复 + 全量回填后。剩余 3 条失败为跨度过大的改写（"断货"↔"缺货"、"送货慢"↔"物流时效差"），属真实检索边界
- **RAG SQL 命中率 0%** 归因查明：`analysis_history` 只存 Agent 最终回答（表格），不存工具调用的 SQL，`search_similar_sql` 正则提取拿不到内容——存储设计缺口（修复路径：data_sources 落库 + 迁移，待下轮）
