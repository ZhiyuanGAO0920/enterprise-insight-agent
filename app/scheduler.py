...
eval_runs 是否已有当天的 canary 记录——没有则子进程跑 run_eval --canary
--save-db（复用 eval.py 同一套引擎），已有则跳过。

幂等设计：以"当天(UTC 日期)是否有 canary 记录"为判据（EvalRun.run_at 为
naive UTC）。与 n8n 触发天然不冲突：无论哪条路径先跑完，另一条都会因
"已有记录"而跳过（eval.py 侧另有 _run_lock 409 并发保护）。
...
async def today_canary_ran() -> bool:
    """当天（UTC 日期）是否已有金丝雀落库记录。"""
    ...
    try:
        res = await session.execute(
            select(func.count())
            .select_from(EvalRun)
            .where(EvalRun.canary.is_(True), EvalRun.run_at >= today_start)
        )
        return res.scalar() > 0
    finally:
        await session.close()
...
async def canary_scheduler_loop() -> None:
    """每日 canary_hour:canary_minute 触发金丝雀评估（幂等：当天已跑过则跳过）。"""
    settings = get_settings()
    hour, minute = settings.canary_hour, settings.canary_minute
    logger.info("金丝雀定时任务启动（每日 %02d:%02d，幂等触发）", hour, minute)
    while True:
        now = datetime.now()
        ...
        try:
            if await today_canary_ran():
                logger.info("当天已有金丝雀记录，跳过（幂等）")
                continue
